...
const ReceivedRequestCard = (props) => {
    ...
    const accept = () => {
        ...
        axios.post("http://localhost:3002/requestReponse", {sender : [name, code], receiver: [props.user, props.code], response: true})//will retrieve a object with friend request for this account
        .then(response => {
    ...
    const refuse = () => {
        ...
        axios.post("http://localhost:3002/requestReponse", {sender : [name, code], receiver: [props.user, props.code], response: false})//will retrieve a object with friend request for this account
        .then(response => {
            console.log(response.data);
        })
        .catch(error => {
